...
const setBg = () => {
  ...
  if (bgNum === 0) {
    ...
    (img.src = `https://raw.githubusercontent.com/Faygo1337/stage1-tasks/assets/images/${timeOfDay}/00.jpg`)
  ) {
    img.src = `https://raw.githubusercontent.com/Faygo1337/stage1-tasks/assets/images/${timeOfDay}/01.jpg`;
  } else {
    img.src = `https://raw.githubusercontent.com/Faygo1337/stage1-tasks/assets/images/${timeOfDay}/20.jpg`;
  }
  img.src = `https://raw.githubusercontent.com/Faygo1337/stage1-tasks/assets/images/${timeOfDay}/${bgNum
    .toString()
    .padStart(2, "0")}.jpg`;
  img.onload = () => {
    body.style.backgroundImage = `url(${img.src})`;
...
async function getQuote() {
  try {
    const urlQuote = "https://zenquotes.io/api/quotes";
    const res = await fetch(urlQuote);
    const data = await res.json();
    quote.textContent = `"${data[playNum++].q}"`;
    author.textContent = data[playNum++].a;
  } catch (err) {
    console.log(`цитаты дали ошибку, но ты не переживай, все будет хорошо`);
    quote.textContent = `цитаты дали ошибку, но ты не переживай, все будет хорошо`;
  }
